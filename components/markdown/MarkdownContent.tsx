...
/**
 * Transform GitHub image URLs to raw.githubusercontent.com URLs
 * Handles:
 * - Relative paths: ./docs/image.png, docs/image.png, /assets/img.png
 * - GitHub blob URLs: https://github.com/user/repo/blob/main/image.png
 * - GitHub raw URLs: already correct, pass through
 */
function transformGitHubImageUrl(src: string, repoInfo?: RepoInfo): string {
  if (!src) return src;

  const branch = repoInfo?.branch ?? 'main';

  // If repoInfo is provided, handle relative URLs
  if (repoInfo) {
    // Check if it's a relative URL (doesn't start with http:// or https://)
    if (!src.startsWith('http://') && !src.startsWith('https://')) {
      // Remove leading ./ or /
      const cleanPath = src.replace(/^\.?\//, '');
      return `https://raw.githubusercontent.com/${repoInfo.owner}/${repoInfo.repo}/${branch}/${cleanPath}`;
    }
  }

  // Transform GitHub blob URLs to raw URLs
  // https://github.com/user/repo/blob/main/path/to/image.png
  // -> https://raw.githubusercontent.com/user/repo/main/path/to/image.png
  ...
  // Also handle user-content URLs (GitHub's CDN for uploaded images)
  // These typically work as-is, so pass through
...
function createMarkdownComponents(repoInfo?: RepoInfo): Components {
  return {
    ...
    img: ({ alt, src, ...props }) => {
      ...
      // Transform GitHub image URLs to raw.githubusercontent.com
      const srcStr = transformGitHubImageUrl(originalSrc, repoInfo);
      ...
      return (
        <div className="my-6 rounded-lg overflow-hidden border border-border shadow-md">
          {/* biome-ignore lint/performance/noImgElement: external images from README need native img */}
          <img src={srcStr} alt={alt ?? ''} className="w-full h-auto" loading="lazy" {...props} />
          {alt && (
